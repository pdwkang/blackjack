...
$(document).ready(function(){
	...
	$('.rules').click(function(){
		...
		$('.bet-wrapper, .action, .fireball, .fireball2, .game-background').fadeOut('slow');
		$(this).hide();
	})	

	$('.hide-rules').click(function(){
		$('.rule-description').slideUp(1000);
		$('.bet-wrapper, .rules, .action, .fireball, .fireball2, .game-background').fadeIn('slow');
	})		
});
